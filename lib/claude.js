...
async function answerFreeQuestion(question, context) {
  const system = `Você é o TEKOA, assistente de família que vive DENTRO desta conversa de WhatsApp. Responda a pergunta do responsável usando SOMENTE o contexto da família fornecido. Seja direto e curto.
...
// depois de apertar [Editar]. Devolve o MESMO formato de extração, para o card
// de confirmação poder ser remontado.
...
Regras:
- Mantenha EXATAMENTE a mesma estrutura de JSON, incluindo o campo "tipo".
...
Responda em JSON estrito, sem texto fora do JSON.`;
  ...
  try {
    const parsed = JSON.parse(match[0]);
    if (!parsed.tipo) parsed.tipo = extraction.tipo;
    return parsed;
  } catch {
    return extraction;
  }
