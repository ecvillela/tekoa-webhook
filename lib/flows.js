...
async function handleFreeText(phone, state, text) {
  ...
  if (!crianca && !viagem) {
    ...
    const answer = await claude.answerFreeQuestion(text, context);
...
async function handleMessage(phone, message) {
  ...
  try {
    ...
    if (state.pendingAction && state.pendingAction.type === 'awaiting_edit_text' && message.type === 'text') {
      ...
      const corrigida = await claude.applyCorrection(original, message.text.body);
    ...
    // extração parcial, e considera uma rodada de esclarecimento suficiente
    // pro MVP — se ainda sobrar algo errado, o usuário sempre pode usar
    // [Editar] no card de confirmação que vem em seguida.
    if (state.pendingAction && state.pendingAction.type === 'awaiting_clarification' && message.type === 'text') {
      const original = state.pendingAction.extraction;
      if (CANCEL_EDIT_INTENT.test(message.text.body)) {
        state.pendingAction = null;
        return await sendText(state, phone, 'Ok, descartei.');
      }
    ...
    if (state.stage === 'new') {
      ...
      await sendText(
        state,
        phone,
        'Pode ir me contando naturalmente — nome e idade dos seus filhos eu vou entendendo pelo caminho, sem formulário. Guardo o que você mandar só pra isso, e você apaga tudo quando quiser só pedindo "apagar meus dados".'
      );
      return await sendText(
        state,
        phone,
    ...
    if (message.type === 'image') {
      ...
      // 2) Documento ambíguo/complexo — várias linhas, várias pessoas, várias
      // datas (ex: planilha de pedidos de comida). Em vez de forçar tudo num
      // card de um evento só (ou escolher uma linha sozinho), pergunta o que
      // falta antes de mostrar qualquer confirmação.
      if (extraction.ambiguo && (extraction.perguntas || []).length) {
        state.pendingAction = { type: 'awaiting_clarification', extraction };
