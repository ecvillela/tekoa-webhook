...
async function sendText(to, body) {
  ...
  if (data.error) console.error(`[whatsapp] sendText falhou: ${JSON.stringify(data.error)}`);  return data;
}
...
async function sendButtons(to, bodyText, buttons) {
  ...
  const data = await r.json();
  if (data.error) console.error(`[whatsapp] sendButtons falhou: ${JSON.stringify(data.error)}`);  return data;
}
